...
4. The legacy Pages workflow and current Pages artifact remain available. Do not run the Pages break-glass workflow while the Worker custom domains are attached.
...
- The HTTP status, redirect chain, response headers, SHA-256 body hash, and UTC capture time for `https://jakh.net/`, `https://www.jakh.net/`, `https://jakh.net/sw.js`, and a known `404` URL.
...
(
  ...
  ./node_modules/.bin/wrangler deployments status --json \
    > "$RECEIPT_DIR/bootstrap-deployment.json"
)
```
...
3. Re-list DNS and compare the restored records field-for-field with the pre-cutover snapshot. Then repeat the legacy HTTP probes until their status, redirects, and body hashes match the saved baseline. DNS propagation is not atomic, so a successful API response alone is not recovery evidence.
4. If the Pages artifact itself must be refreshed after DNS is restored, dispatch `Legacy GitHub Pages (break-glass only)` from protected `main`, enter `DEPLOY LEGACY GITHUB PAGES`, and provide the approved incident/change reference.
